...
/* Android 上崩溃转储不归应用管： tombstone 由 debuggerd 写到 /data/tombstones/，
 * 应用私有目录读不到，日志要靠 logcat / Crashlytics 这类系统通道。
 * 所以这里和 Linux/FreeBSD 版一样返回空路径 —— CrashHandler 的哨兵文件机制
 * （判断"上次是不是没正常退出"）仍然照常工作，只是没有可上传的日志文件。 */
